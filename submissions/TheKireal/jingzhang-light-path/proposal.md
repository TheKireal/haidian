...
- **南段—中段慢行衔接与引导升级**：南段（北京北站—知春路）与中段（知春路—清华东路）之间现状被建筑和道路分隔（可核实现状）；现场已设公园导览地图牌，并有人行/自行车天桥跨越道路构成慢行衔接。过桥后公园步道中断，行人进入城市社区马路，路线连续性在此断裂（现场观察）。拆迁打通需待权属和规划核实，短期内不作为实施路径。在社区马路增设地面指引和标识牌涉及交通管理与城市规划协调，须取得交通部门和社区同意，这也是该断点尚未系统治理的原因之一。概念建议：不拆不建，在现有天桥和导览系统基础上升级包容性引导——天桥加触感铺装与盲道衔接，导览牌补盲文版与语音扫码解说；重点在过桥后步道中断处设置醒目标识牌与地面指引（脉搏三色线+箭头+距离标注+中段方向），引导行人穿过社区马路重新接入公园步道；天桥入口加低压自发光夜间标识。视障使用者可通过触感+语音连续导航，健视使用者可通过视觉标识识别路线。所有社区马路范围内的地面标识和标牌实施前须取得交通部门许可和社区同意。成本低、可迭代 [depth:transport_organization]。
